...
const Modal = ({ onClick }) => {
    ...
    useEffect(() => {
        ...
        dispatchState({ type: TOTAL_PRICE, payload: priceSum })
    }, [context.items])

    return (
        <Wrapper>
            <Container contextLength={context.items.length}>
                {state.modalSize &&
                    <UnorderdList contextLength={context.items.length}>
                        {context.items.map((el) => {
                            return <ModalItem id={el.id} title={el.title} price={el.price} amount={el.amount}></ModalItem>
                        })}
                    </UnorderdList>
                }
                <div>
